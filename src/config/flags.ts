...
export const FLAGS = {
  ...
  /** DevTools console commands (`window.chikReset`, etc). Defaults to ON in dev
   *  builds and OFF in production so a stock prod deploy doesn't expose reset
   *  utilities to end-user consoles. Override either way with
   *  `VITE_DEVTOOL_COMMANDS_ENABLED=true|false` in `.env.local`. */
  devtoolCommandsEnabled: parseBool(
    import.meta.env.VITE_DEVTOOL_COMMANDS_ENABLED,
    !!import.meta.env.DEV,
  ),
} as const;
